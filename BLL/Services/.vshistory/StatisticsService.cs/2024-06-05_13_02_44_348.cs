    ...
    public class StatisticsService: BaseService
    {
        ...
        public CollectionPointStatistics CreatePointStatistics(int point, DateOnly startDate, DateOnly endDate)
        {
            ...
            var operations = dbContext.Operation
                 .Where(o => o.CollectionPointID == point && DateOnly.FromDateTime(o.Time.DateTime) >= startDate && DateOnly.FromDateTime(o.Time.DateTime) <= endDate)
                 .ToList();
            ...
            CollectionPointStatistics collectionPointStatistics = new()
            {
                ...
                MostCollectedType = operations
                    ...
                    .Select(g => g.Key)
                    .FirstOrDefault(),
                Attendance = operations.Select(o => o.UserID).Distinct().Count(),
