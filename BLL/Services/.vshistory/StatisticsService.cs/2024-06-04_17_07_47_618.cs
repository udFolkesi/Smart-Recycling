    ...
    public class StatisticsService: BaseService
    {
        ...
        public CollectionPointStatistics CreatePointStatistics(int point, DateOnly startDate, DateOnly endDate)
        {
            List<Operation> operations = dbContext.Operation
                .Where(o => o.CollectionPointID == point && o.Time.Date >= startDate.ToDateTime(TimeOnly.MinValue) && o.Time.Date <= endDate.ToDateTime(TimeOnly.MaxValue))
                .ToList();

            List<Operation> filteredOperations = operations
                .Where(o => DateOnly.FromDateTime(o.Time.Date) >= startDate && DateOnly.FromDateTime(o.Time.Date) <= endDate)
                .ToList();

            CollectionPointStatistics collectionPointStatistics = new()
            {
                Collected = filteredOperations.Sum(o => o.Weight),
                Recycled = 0,
                MostCollectedType = filteredOperations
                    .GroupBy(o => o.TrashType)
                    .OrderByDescending(g => g.Sum(o => o.Weight))
                    .Select(g => g.Key)
                    .FirstOrDefault(),
                Attendance = filteredOperations.Select(o => o.UserID).Distinct().Count(),
