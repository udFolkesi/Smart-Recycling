...
/*            var distanceToPoints =
            ...
            var recyclingPoints = dbContext.RecyclingPoint.Where(p => p.RecyclingType == trashType).ToList();

            foreach (var point in recyclingPoints)
            {
                double distance = await GetDistance(string.Join("2C", location.Split(' ')), string.Join("2C", point.Location.Split(' ')));
