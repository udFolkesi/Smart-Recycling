...
namespace SmartRecycling.Controllers
{
    ...
    public class CollectionPointController : Controller
    {
        ...
        {
            await dbContext.CollectionPointStatistics.AddRangeAsync(pointStatistics);
            await dbContext.SaveChangesAsync();
        ...
        {
            var statistics = dbContext.CollectionPointStatistics
            byte[] pdfBytes = statisticsService.GeneratePdfContent(point, startDate, endDate);
            MemoryStream stream = new MemoryStream(pdfBytes);
            stream.Position = 0;

            return new FileStreamResult(stream, "application/pdf")
            {
                FileDownloadName = $"report-for:{startDate}-{endDate}.pdf"
            };
        }
    }
}